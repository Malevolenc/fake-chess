...
export default function genAllLegalMoves(pieceCoords, chessBoardArray, currentTurn, kingCoords, moveNumber){
    let allLegalMoves = []
    
    for (const pieceCoord of pieceCoords){

        let [iRow, iColumn] = Coordinate.coordsToIndices(pieceCoord)
        let pseudoLegalPieceMoves = genPseudoLegalPieceMoves(chessBoardArray[iRow][iColumn], chessBoardArray, iRow, iColumn, currentTurn, moveNumber)
